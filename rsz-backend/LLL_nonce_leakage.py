...
def analyze(fix_bits=56):
    """API için kullanılacak fonksiyon"""
    try:
        # Örnek bir private key ve nonce oluştur
        pvk = random.SystemRandom().randint(1, 2**256)
        k = random.SystemRandom().randint(1, 2**256)
        
        # Nonce'un ilk fix_bits bitini al
        mask = (1 << fix_bits) - 1
        leaked_bits = k & mask
        
        return {
            'private_key': hex(pvk),
            'original_nonce': hex(k),
            'leaked_bits': fix_bits,
            'leaked_nonce_part': hex(leaked_bits),
            'remaining_bits': 256 - fix_bits
        }
        
    except Exception as e:
        raise Exception(f"Error in nonce leakage analysis: {str(e)}")
